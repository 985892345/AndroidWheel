package com.g985892345.android.extensions.rxpermissions
...
private fun performRequestPermission(
  ...
  val sp = context.getSharedPreferences("RxPermissions", Context.MODE_PRIVATE)
  val isNeverShowSpKey = "isNeverShow_$permissionsRequired"
  var isNeverShow = sp.getBoolean(isNeverShowSpKey, false)
  if (isNeverShow) {
    ...
    sp.edit {
      putBoolean(isNeverShowSpKey, false)
    }
  }
    ...
    return AlertDialog.Builder(context).apply {
      ...
      if (builder.isShowNeverNotice) {
        setNegativeButton("不再提示") { _, _ ->
          sp.edit {
            putBoolean(isNeverShowSpKey, true)
          }
          builder.doOnCancel?.invoke()
        }
