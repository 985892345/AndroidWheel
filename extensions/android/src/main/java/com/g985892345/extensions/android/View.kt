...
fun View.setOnSingleClickListener(interval: Long = 500, click: (View) -> Unit) {
  setOnClickListener {
    val tag = getTag(R.id.extensions_single_click_id) as? Long
    if (System.currentTimeMillis() - (tag ?: 0L) > interval) {
      click(it)
    }
    it.setTag(R.id.extensions_single_click_id, System.currentTimeMillis())
  }
}
...
fun View.setOnDoubleClickListener(interval: Long = 500, click: (View) -> Unit) {
  setOnClickListener {
    val tag = getTag(R.id.extensions_double_click_id) as? Long
    if (System.currentTimeMillis() - (tag ?: 0L) < interval) {
      click(it)
    }
    it.setTag(R.id.extensions_double_click_id, System.currentTimeMillis())
  }
}