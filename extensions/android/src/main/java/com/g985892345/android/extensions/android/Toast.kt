package com.g985892345.android.extensions.android

import android.content.Context
import android.os.Handler
import android.os.Looper
import android.util.Log
import android.widget.Toast

/**
 * ...
 * @author 985892345 (Guo Xiangrui)
 * @email 2767465918@qq.com
 * @date 2022/3/7 17:58
 */

/**
 * TODO 可以在 debug 包下控制台筛选：toast，查看 toast 调用堆栈信息
 */

/**
 * 已自带处于其他线程时自动切换至主线程发送
 */
fun toast(s: CharSequence?) {
  ToastUtil.show(appContext, s, Toast.LENGTH_SHORT)
}

fun toastLong(s: CharSequence?) {
  ToastUtil.show(appContext, s, Toast.LENGTH_LONG)
}

fun String.toast() = toast(this)
fun String.toastLong() = toastLong(this)

object ToastUtil {
  /**
   * 已自带处于其他线程时自动切换至主线程发送
   */
  fun show(
    context: Context,
    text: CharSequence?,
    duration: Int
  ) {
    if (text == null) return
    if (Thread.currentThread() !== Looper.getMainLooper().thread) {
      Handler(Looper.getMainLooper()).post { showToastInternal(context, text, duration) }
    } else {
      showToastInternal(context, text, duration)
    }
  }

  /**
   * 用于具体的项目设置 toast
   */
  fun setToast(action: (Context, CharSequence, Int) -> Unit) {
    mToast = action
  }

  private var mToast: (Context, CharSequence, Int) -> Unit = { context, text, duration ->
    Toast.makeText(context, text, duration).show()
  }

  private fun showToastInternal(
    context: Context,
    text: CharSequence,
    duration: Int
  ) {
    if (isDebuggableBuild) {
      val throwable = Throwable() // 获取堆栈信息
      val path = throwable.stackTrace
        .toMutableList()
        .apply { removeFirst() } // 从堆栈中去掉当前方法
        .filter {
          // 第一次先筛选出是自己代码中的方法栈
          !it.isNativeMethod
              && it.fileName != null
              && it.fileName.endsWith(".kt")
              && it.className.startsWith("com.")
        }.after {
          // 第二次筛选出不必要的方法栈
          !it.className.contains(".base.") // 筛掉 base 包
              && !it.fileName.startsWith("Base")
              && !it.fileName.matches(Regex("[tT]oast")) // 去掉一些工具类
        }.joinToString(separator = " <- ") {
          "(${it.fileName}:${it.lineNumber})"
        }
      Log.d("toast", "toast: text = $text   path: $path")
    }
    mToast.invoke(context, text, duration)
  }

  /**
   * 寻找第一个满足条件后的子数组
   */
  private fun <T> List<T>.after(first: (T) -> Boolean): List<T> {
    val list = ArrayList<T>()
    var isFound = false
    forEach {
      if (isFound) {
        list.add(it)
      } else {
        isFound = first.invoke(it)
      }
    }
    return list
  }
}
