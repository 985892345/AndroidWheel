...
fun View.setOnSingleClickListener(interval: Long = 500, click: (View) -> Unit) {
  setOnClickListener {
    val tag = getTag(R.id.id_single_click) as? Long
    if (System.currentTimeMillis() - (tag ?: 0L) > interval) {
      click(it)
    }
    it.setTag(R.id.id_single_click, System.currentTimeMillis())
  }
}
...
fun View.setOnDoubleClickListener(interval: Long = 500, click: (View) -> Unit) {
  setOnClickListener {
    val tag = getTag(R.id.id_double_click) as? Long
    if (System.currentTimeMillis() - (tag ?: 0L) < interval) {
      click(it)
    }
    it.setTag(R.id.id_double_click, System.currentTimeMillis())
  }
}