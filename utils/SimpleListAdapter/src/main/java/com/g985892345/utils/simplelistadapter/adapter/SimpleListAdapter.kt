package com.g985892345.utils.simplelistadapter.adapter
...
class SimpleListAdapter internal constructor(
) : ListAdapter<IAdapterData, SimpleListViewHolder>(itemCallback), ISimpleList {
  ...
  override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SimpleListViewHolder {
    ...
    view.setTag(R.id.controller_by_viewHolder, controller)
  ...
  override fun onBindViewHolder(
    ...
    payloads.forEach {
      when (it) {
        is ISimpleListAdapterHelper.ViewHolderCallback -> it.call(holder)
        else -> isOnBind = true
      }
    }
  ...
  override fun onFailedToRecycleView(holder: SimpleListViewHolder): Boolean {
    return holder.controller.onFailedToRecycleView(holder)
  }

  override fun onViewAttachedToWindow(holder: SimpleListViewHolder) {
    super.onViewAttachedToWindow(holder)
    holder.controller.onViewAttachedToWindow(holder)
  }

  override fun onViewDetachedFromWindow(holder: SimpleListViewHolder) {
